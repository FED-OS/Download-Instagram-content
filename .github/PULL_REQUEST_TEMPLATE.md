...
- [ ] My code follows the project conventions (engines return `DownloadResult`, network calls are wrapped in try/except, no API keys).
- [ ] I added/updated a platform only if it works on **public** content without login (or wired through the existing cookies mechanism).
- [ ] I did **not** commit any secrets, cookies, downloaded media, or `__pycache__`.
